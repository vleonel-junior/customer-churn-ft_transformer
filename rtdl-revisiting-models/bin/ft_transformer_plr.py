...
class TokenizerPLR(nn.Module):
    ...
    def __init__(
        ...
        d_token: int,
        bias: bool,
        X_train: ty.Optional[Tensor] = None,
...
if __name__ == "__main__":
    ...
    args['model'].setdefault('embedding_type', 'P-LR')
    ...
    # Créer le modèle avec embeddings P-LR
    model = TransformerPLR(
        d_numerical=0 if X_num is None else X_num['train'].shape[1],
        categories=lib.get_categories(X_cat),
        d_out=D.info['n_classes'] if D.is_multiclass else 1,
        X_train=X_num['train'] if X_num is not None else None,
        **args['model'],
    ).to(device)
